/**
 * Immediate fix script for stuck pending transactions
 * This script marks OLD pending transactions as failed (since they've been pending for days/weeks)
 * 
 * Usage: npx ts-node fix-stuck-pending-transactions.ts
 */
...
async function fixStuckPendingTransactions(): Promise<void> {
  try {
    ...
    for (const transaction of pendingTransactions) {
      ...
      if (timeDiffHours > 0.5) {
        ...
        const failureReason = timeDiffDays > 2 
          ? `Payment stuck in pending state for ${timeDiffDays.toFixed(0)} days - marked as failed`
