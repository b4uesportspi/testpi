...
async function safeCleanupPackages() {
  ...
  try {
    ...
    console.log('Starting safe package cleanup...');
    
    // 1. Check for duplicate PUBG 60 UC packages
    ...
    if (duplicatePackages.rows.length > 1) {
      console.log(`Found ${duplicatePackages.rows.length} duplicate PUBG 60 UC packages.`);
      
      // Keep the first one (oldest), identify the rest for deletion
      const packagesToKeep = duplicatePackages.rows[0];
      const packagesToDelete = duplicatePackages.rows.slice(1);
      
      console.log(`Keeping package: ${packagesToKeep.id} (created: ${packagesToKeep.created_at})`);
      
      for (const pkg of packagesToDelete) {
        console.log(`Processing duplicate package: ${pkg.id} (created: ${pkg.created_at})`);
        
        // Check if any transactions reference this package
        const transactionCount = await client.query(
          'SELECT COUNT(*) as count FROM transactions WHERE package_id = $1',
          [pkg.id]
        );
        
        if (transactionCount.rows[0].count > 0) {
          console.log(`  Found ${transactionCount.rows[0].count} transactions referencing this package.`);
          console.log(`  Updating transactions to reference the kept package instead...`);
          
          // Update transactions to reference the kept package
          const updateResult = await client.query(
            'UPDATE transactions SET package_id = $1 WHERE package_id = $2',
            [packagesToKeep.id, pkg.id]
          );
          
          console.log(`  Updated ${updateResult.rowCount} transactions.`);
        } else {
          console.log(`  No transactions reference this package.`);
        }
        
        // Now safe to delete the duplicate package
    ...
    const mlbbPackages = await client.query(
      'SELECT id, name, in_game_amount, usdt_value FROM packages WHERE game = $1 ORDER BY in_game_amount',
      ['MLBB']
    );
    
    // Correct pricing for MLBB packages (based on standard market rates)
    ...
    for (const pkg of mlbbPackages.rows) {
      ...
      if (correctPrice && pkg.usdt_value !== correctPrice) {
        await client.query(
          'UPDATE packages SET usdt_value = $1, updated_at = NOW() WHERE id = $2',
          [correctPrice, pkg.id]
        );
    ...
    const pubgPackages = await client.query(
      'SELECT id, name, usdt_value FROM packages WHERE game = $1 ORDER BY in_game_amount',
      ['PUBG']
    );
    ...
    const cocPackage = await client.query(
      'SELECT id, usdt_value FROM packages WHERE game = $1 AND name = $2',
      ['COC', 'Gold Pass']
    );
    ...
    console.log('\nSafe package cleanup and pricing verification completed successfully!');
    
  } catch (error) {
    console.error('Error during safe package cleanup:', error);
  } finally {
    await pool.end();
  }
}
