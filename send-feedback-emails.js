...
import { sendFeedbackRequestEmail } from "./server/services/email.ts";
...
async function sendFeedbackEmailsToAllSuccessfulPurchasers() {
  ...
  try {
    ...
    console.log(`?? Found ${usersWithCompletedPurchases.length} completed transactions`);
    ...
    console.log(`?? Found ${uniqueUsers.length} unique users with completed purchases`);
    ...
    for (const user of uniqueUsers) {
      try {
        console.log(`?? Sending feedback email to ${user.username} (${user.email})`);
        ...
        if (success) {
          successCount++;
          console.log(`? Successfully sent feedback email to ${user.username}`);
        } else {
          failureCount++;
          console.log(`? Failed to send feedback email to ${user.username}`);
        }
        ...
        console.error(`? Error sending feedback email to ${user.username}:`, error);
        failureCount++;
      }
    }
